...
import movieImage from './movie-list-image.png';
import './App.css';
...
function App() {
  ...
  const renderStars = (rating) => {
    ...
    for (let i = 0; i < fullStars; i++) {
      starImages.push(<img key={`full-${i}`} src="/full-star.png" alt="full star" className="star" />);
    }
    if (halfStar) {
      starImages.push(<img key="half" src="/half-star.png" alt="half star" className="star" />);
    }
    for (let i = 0; i < emptyStars; i++) {
      starImages.push(<img key={`empty-${i}`} src="/empty-star.png" alt="empty star" className="star" />);
    }
  ...
  // We're going to be using effects for this. First, we need to retrieve the movie list
  useEffect(() => {
    fetch('/movie_results.txt') // Fetch the file from the public folder
      .then(res => res.text())  // Read the response as text
      .then(text => {
        const lines = text.trim().split('\n');  // Split the content by newlines
        const headers = lines[0].replace(/"/g, '').split(',');  // Parse headers and remove quotes
        const movieData = lines.slice(1).map(line => {
          const values = line.match(/(".*?"|[^",\s]+)(?=\s*,|\s*$)/g).map(v => v.replace(/^"|"$/g, ''));
          const movie = {};
          headers.forEach((header, i) => {
            if (header === 'avgRating') {
              movie[header] = parseFloat(values[i]);  // Convert avgRating to a number
            } else {
        ...
        setMovies(movieData);  // Set the parsed movies data
      })
